...
const SidebarM = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:8080/api/category') // Sử dụng axios.get thay cho fetch
      .then((response) => {
        if (response.status === 200) {
          return response.data; // Axios tự động chuyển đổi JSON
        } else {
...
<div className="col-xs-12 col-sm-12 col-md-3 sidebar">
        ...
        <ul className="list-group list-group-flush">
          ...
          {categories.map((category) => (
            <li key={category.id} className="list-group-item">
              <Link to={`/products/${category.id}`} state={{ listCategory: category.listCategory }}>{category.type_category}</Link>
            </li>
          ))}
        </ul>
      </div>
          ...
          <div className="item">
            <div className="products">
              ...
              <div className="cart clearfix animate-effect">
                <button type="button" className="btn" style={{ background: "#319F43 ", color: '#fff' }}>Thêm vào giỏ hàng</button>
              </div>
            </div>
          </div>
          <div className="item">
      ...
      <div className="sidebar-widget outer-bottom-small bg-white mt-4" >
        ...
        <div className="sidebar-widget-body outer-top-xs">
          <div
            className="owl-carousel sidebar-carousel special-offer custom-carousel owl-theme outer-top-xs"
          >
            <div className="item">
