...
function ListShopWait() {
  ...
  useEffect(() => {
    if (Array.isArray(data)) {
      const matchingShop = data.filter((shop) =>
        shop.status === 0)
      ...
      <div className={style.listBill}>
        <div className={style.table}>
          <div className={style.tableHeading}>
            ...
            <label className={style.column}>Mã cửa hàng</label>
          ...
          {ListShopWait.map((value, index) => (
            <div key={index} className={style.tableBody}>
              ...
              <label className={style.column}>{value.accountShop.username}</label>
              <label className={style.column}>
              ...
              <label className={style.column} onClick={()=>{dispatch(getIdShop(value.id));}}>
